...
async function fetchJson(url, options) {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      lastFetchError = "HTTP " + response.status + " " + text;
    ...
    console.error(error);
    if (!lastFetchError) {
      lastFetchError = error && error.message ? error.message : "Network error";
    }
    return null;
  }
}
...
async function unlockWithPin(pinInput) {
  ...
  if (!remoteData) {
    var detail = lastFetchError ? lastFetchError : "Cloud data unavailable.";
    setPinError(detail);
    return false;
  }
